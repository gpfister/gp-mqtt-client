...
export class GPMqttController extends EventEmitter {
    ...
    private _protocol: 'wss' | 'ws' | 'mqtt' | 'mqtts' | 'tcp' | 'ssl' | 'wx' | 'wxs';
     ...
     */
    constructor(hostname: string, port: number, protocol: 'wss' | 'ws' | 'mqtt' | 'mqtts' | 'tcp' | 'ssl' | 'wx' | 'wxs', autoConnect: boolean, verbose = false, debug = false) {
        super();
    ...
    connect() {
        ...
        this._client.on('connect', (_) => {
            this._logger.info(`[MQTT Controller] Connected to ${this._hostname}:${this._port} with client id ${mqttClientId} bytes`);
            this.emit('connected');
        ...
        if (!this._topic) {
            ...
            if (this._client && this._client.connected) {
                ...
                this._client.on('message', async (topic, message) => {
                    ...
                    if (options.useBase64Decoding) {
                        this._logger.debug('[MQTT Controller] Apply base64 decoding');
                    }
                    ...
                    if (options.outputPrettyJson) {
                        this._logger.debug('[MQTT Controller] Apply JSON pretty print');
                    }
